  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm mt-6">
        ...
        <p className="text-sm text-gray-900 mt-1">{goal.createdAt.slice(0, 10)}</p>

        <p className="text-sm text-gray-500 mt-4">Text</p>
        <p className="text-base text-gray-900 mt-1 whitespace-pre-wrap break-all">{goal.text}</p>
      ...
      {openEdit ? (
        <GoalEditModal
          ...
          onSave={async (newText) => {
            try {
              await dispatch(updateGoal({ id: goal._id, data: { text: newText } })).unwrap();
              setOpenEdit(false);
            } catch {
